...
describe 'filebeat::default' do
  ...
  context 'windows' do
    ...
    it 'create prospector directory C:/opt/filebeat/filebeat-5.6.4-windows-x86_64/conf.d' do
      expect(chef_run).to create_directory('C:/opt/filebeat/filebeat-5.6.4-windows-x86_64/conf.d')
    end

    it 'configure C:/opt/filebeat/filebeat-5.6.4-windows/filebeat.yml' do
      expect(chef_run).to create_file('C:/opt/filebeat/filebeat-5.6.4-windows-x86_64/filebeat.yml')
    end
    ...
    it "has correct default['filebeat']['conf_dir']" do
      expect(node['filebeat']['conf_dir']).to eq('C:/opt/filebeat/filebeat-5.6.4-windows-x86_64')
    end

    it "has correct default['filebeat']['config']['filebeat.registry_file']" do
      expect(node['filebeat']['config']['filebeat.registry_file']).to eq('C:/opt/filebeat/filebeat-5.6.4-windows-x86_64/registry')
    end

    it "has correct default['filebeat']['config']['filebeat.config_dir']" do
      expect(node['filebeat']['config']['filebeat.config_dir']).to eq('C:/opt/filebeat/filebeat-5.6.4-windows-x86_64/conf.d')
    end
  end
end
